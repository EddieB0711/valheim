...
#define VALIDATE(result) if (result) {} else { valheim_deinitContext( context, allocator ); return false; }
	...
	VALIDATE(valheim_initQueue(sizeof(valheim_VulkanJob), allocator, &context->highPriority));
	VALIDATE(valheim_initQueue(sizeof(valheim_VulkanJob), allocator, &context->mediumPriority));
	VALIDATE(valheim_initQueue(sizeof(valheim_VulkanJob), allocator, &context->lowPriority));
	VALIDATE(valheim_initMap(&context->pipelineHandles, sizeof(valheim_VulkanPipeline), 1024, valheim_hash, allocator));

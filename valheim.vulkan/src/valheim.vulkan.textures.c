...
void valheim_initTextureManager(valheim_VulkanContext *context) {
	valheim_initArray(context->textureManager.images, VALHEIM_KIBIBYTE(1), context->allocator);
	valheim_initArray(context->textureManager.imageMemory, VALHEIM_KIBIBYTE(1), context->allocator);
	valheim_initArray(context->textureManager.imageViews, VALHEIM_KIBIBYTE(1), context->allocator);
	valheim_initArray(context->textureManager.imageSizes, VALHEIM_KIBIBYTE(1), context->allocator);
	valheim_initArray(context->textureManager.samplers, VALHEIM_KIBIBYTE(1), context->allocator);
	valheim_initArray(context->textureManager.freeList, VALHEIM_KIBIBYTE(1), context->allocator);
...
void valheim_deinitTextureManager(valheim_VulkanContext *context) {
	for (u32 i = 0; i < context->textureManager.images.length; i++) {
...
b8 valheim_createTexture(valheim_VulkanContext *context, const char *filename, valheim_VulkanTexture *outTexture) {

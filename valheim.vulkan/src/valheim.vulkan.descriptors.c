...
void valheim_deinitDescriptorSetManager(valheim_VulkanContext *context, valheim_Allocator *allocator) {
	for (u32 iSet = 0; iSet < context->descriptorSetManager.freeList.length; ++iSet) {
		if (!context->descriptorSetManager.freeList.data[iSet]) {
			...
			valheim_free(allocator, context->descriptorSetManager.descriptorSets.data[iSet]);
		}
	}
...
b8 valheim_initDescriptors(valheim_VulkanContext *context, valheim_DescriptorSetCreateInfo *createInfo, valheim_VulkanDescriptorSet *descriptorSet, valheim_Allocator *allocator) {
	...
	VkDescriptorSet *descriptorSets = valheim_allocate(allocator, sizeof(VkDescriptorSet) * context->imageCount);
